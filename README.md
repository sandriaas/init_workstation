...
| Name | IP | Hostname | RAM | Purpose | Pattern |
|------|-----|----------|-----|---------|---------|
| `vm_1` | 192.168.122.50 (static) | `ubuntu-server` | 4 GB | Primary headless server (Dokploy, cloudflared) | Cloud image + cloud-init seed |
| `vm_2` | 192.168.122.51 (DHCP reserved) | `vm-2` | 6 GB | Secondary VM (VNC + Cockpit-accessible) | Cloud image + cloud-init seed |

Both VMs are provisioned from the **Ubuntu noble cloud image** (not the live-server installer ISO) — boot in ~30s, no installer to run. Cloud-init reads the seed ISO on first boot to set hostname, user, SSH, and network.

- vm_1: see `configs/vm.conf` (generated by `scripts/phase2.sh`) and `generated-vm/vm_1.conf`
- vm_2: see `generated-vm/vm_2.conf` + `generated-vm/vm_2-README.md` (`recreate_vm2.sh` reproduces it)
- Lessons learned (autoinstall vs cloud-image, UFW DHCP gotcha, etc.): `generated-vm/vm_2-LESSONS.md`

### Critical: UFW must allow DHCP on virbr0

Without this, libvirt VMs silently fail to get DHCP leases. Run once per host:
