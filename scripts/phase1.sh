...
print_final_summary() {
  ...
  echo ""
  echo -e "${BOLD}╔══════════════════════════════════════════════════════════════╗${RESET}"
  echo -e "${BOLD}║              PHASE 1 COMPLETE — CONNECTION GUIDE            ║${RESET}"
  echo -e "${BOLD}╚══════════════════════════════════════════════════════════════╝${RESET}"
  echo ""
  echo -e "${BOLD}Tunnel:${RESET} ${TUNNEL_HOST_SAVED}"
  echo -e "${BOLD}User:${RESET}   ${TUN_USER}"
  echo ""
  echo -e "${BOLD}── ~/.ssh/config entry (same for all platforms) ──${RESET}"
  echo ""
  echo "  Host minipc"
  echo "    HostName ${TUNNEL_HOST_SAVED}"
  echo "    ProxyCommand websocat -E --binary - wss://%h"
  echo "    User ${TUN_USER}"
  echo ""
  echo -e "${BOLD}── Install websocat per platform ──${RESET}"
  echo ""
  ...
  echo ""
  echo -e "${BOLD}── Android / Termux one-liner (paste in Termux) ──${RESET}"
  echo ""
  echo "  pkg update -y && pkg install -y openssh websocat && \\"
  echo "  mkdir -p ~/.ssh && printf 'Host minipc\\n  HostName ${TUNNEL_HOST_SAVED}\\n  ProxyCommand websocat -E --binary - wss://%%h\\n  User ${TUN_USER}\\n' >> ~/.ssh/config && \\"
  echo "  chmod 600 ~/.ssh/config"
  echo ""
  echo -e "${BOLD}── Connect from anywhere ──${RESET}"
  echo ""
  echo -e "  ${GREEN}ssh minipc${RESET}"
  echo ""
