...
prompt_resource_and_vm_basics() {
  ...
  _existing_count="$(virsh list --all --name 2>/dev/null | grep "^vm_" | wc -l)" || _existing_count=0
  _existing_count="${_existing_count//[^0-9]/}"
  [ -z "$_existing_count" ] && _existing_count=0
...
write_vm_conf() {
  ...
  mkdir -p "${VM_CONF_DIR}"
  # Write conf — use a temp approach for the password hash line to avoid $6 expansion
  local _pw_hash_escaped="${VM_PASSWORD_HASH:-}"
  cat > "$VM_CONF" <<EOF
...
VM_USER="${VM_USER}"
EOF
  # Password hash contains $6$ which must not be expanded by bash
  echo "# SHA-512 hash of VM user password (used by Ubuntu autoinstall — never store plaintext)" >> "$VM_CONF"
  printf 'VM_PASSWORD_HASH="%s"\n' "$_pw_hash_escaped" >> "$VM_CONF"
  cat >> "$VM_CONF" <<EOF
...
VM_NET_IFACE="${VM_NET_IFACE:-enp1s0}"
...
select_or_create_conf() {
  ...
  # Remove empty entries from mapfile
  local -a _clean=()
  local _e; for _e in "${EXISTING[@]+"${EXISTING[@]}"}"; do [ -n "$_e" ] && _clean+=("$_e"); done
  EXISTING=("${_clean[@]+"${_clean[@]}"}")
